  ...
  <div class="app-layout">
    ...
    <aside class="sidebar" :class="{ open: sidebarOpen }">
      ...
      <nav class="sidebar-nav" aria-label="Student navigation">
        <NuxtLink :to="{ path: '/student/dashboard', query: { tab: 'overview' } }" class="nav-link">
          <UIcon name="i-lucide-layout-dashboard" class="nav-icon" />
          <span>Overview</span>
        </NuxtLink>
        <NuxtLink :to="{ path: '/student/dashboard', query: { tab: 'notes' } }" class="nav-link">
          <UIcon name="i-lucide-sticky-note" class="nav-icon" />
          <span>Notes</span>
        </NuxtLink>
        <NuxtLink :to="{ path: '/student/dashboard', query: { tab: 'assignments' } }" class="nav-link">
          <UIcon name="i-lucide-clipboard-list" class="nav-icon" />
          <span>Assignments</span>
        </NuxtLink>
        <NuxtLink :to="{ path: '/student/dashboard', query: { tab: 'exams' } }" class="nav-link">
          <UIcon name="i-lucide-pencil" class="nav-icon" />
          <span>Exams</span>
        </NuxtLink>
        <NuxtLink :to="{ path: '/student/dashboard', query: { tab: 'tests' } }" class="nav-link">
          <UIcon name="i-lucide-square-check" class="nav-icon" />
          <span>Tests</span>
        </NuxtLink>
        <NuxtLink :to="{ path: '/student/dashboard', query: { tab: 'results' } }" class="nav-link">
          <UIcon name="i-lucide-bar-chart-3" class="nav-icon" />
          <span>Results</span>
        </NuxtLink>
      </nav>
